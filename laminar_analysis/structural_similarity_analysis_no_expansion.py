...
# drop unused columns
left_df = relative_power[relative_power['column'] == 'center'].drop(['Unnamed: 0', 'Unnamed: 0.1', 'channel index',
                                                                     'column'], axis=1)
# treat the image shifting problem like convolution?
# define template for each site
max_site_similarity = pd.DataFrame()
for temp_site in sites:
    ...
    for site in other_sites[1:]:
        ...
        shifted_images_short = shifted_images[:, :, int(len(image[:, 0]) / 2):-int(len(image[:, 0]) / 2)]
        shifted_templates_short = shifted_templates[:, :, int(len(image[:, 0]) / 2):-int(len(image[:, 0]) / 2)]
        ...
        if max_similarity > 0.6:
            ...
            if shift_index < len(image[:, 0]):
                ...
                template = np.vstack((template_overlap, template_overhang))
                if len(template) < 22:
                ...
                template_overhang_above = template[:shift_index - len(image[:, 0]) + 1, :]
                template = np.vstack((template_overhang_above, template_overlap))
                if len(template) < 22:
                    raise ValueError("template too short")
